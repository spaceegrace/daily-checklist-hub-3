/* =========================================================
   ...
   Restored to behave like V25 with tabs, trackers, hops, graphs, and insights fully working
========================================================== */
...
(function (window) {
    ...
    function setupButtons() {
        ...
        document.querySelectorAll('.mood-btn').forEach(btn => {
            btn.addEventListener('click', ()=> addLog('moodLog',{type:'mood', val:btn.dataset.mood, icon:moodEmojis[btn.dataset.mood], fullDate:currentFullDate()}));
        });
        document.querySelectorAll('.drop-btn').forEach((btn,i)=>{
            btn.addEventListener('click', ()=>{
                ...
                else{ btn.classList.add('active'); pondData.waterCount=i+1; addLog('waterLog',{type:'water', val:pondData.waterCount, icon:'💧', fullDate:currentFullDate()}); }
                saveAndRefresh();
            });
        });
    }
    ...
    // ======================== TRACKER FUNCTIONS ========================
    function addSugar(){ var val=document.getElementById('sugarInput').value; if(val) addLog('sugarLog',{type:'sugar', val:parseFloat(val), fullDate:currentFullDate()}); }
    function addCarb(){ var val=document.getElementById('carbInput').value; if(val) addLog('carbLog',{type:'carb', val:parseFloat(val), fullDate:currentFullDate()}); }
    function addInsulin(){ var val=document.getElementById('insulinInput').value; if(val) addLog('insulinLog',{type:'insulin', val:parseFloat(val), fullDate:currentFullDate()}); }
    function addSleep(){ /* implement sleep input */ }
    function addStress(level){ addLog('stressLog',{type:'stress', val:level, score:5, fullDate:currentFullDate()}); }
    function addEnergy(level){ addLog('energyLog',{type:'energy', val:level, score:5, fullDate:currentFullDate()}); }
    function addSymptom(name){ addLog('symptomLog',{type:'symptom', val:name, fullDate:currentFullDate()}); }
    function addExerciseFromInput(type,intensity){ addLog('exerciseLog',{type:'exercise', val:type, intensity:intensity, fullDate:currentFullDate()}); }
    ...
    function renderTasks(){
        ...
        pondData.daily.forEach(item=>{
            var li=document.createElement('li'); li.textContent=`${item.text} [${item.priority}]`; list.appendChild(li);
    ...
    function renderInsightPanel(){
        ...
        box.innerHTML=''; if(pondData.sugarLog.length===0){ box.textContent='Log some health data to see insights'; return; }
        var ul=document.createElement('ul');
        var avgSugar=average(pondData.sugarLog,l=>l.val).toFixed(1);
        var li=document.createElement('li'); li.textContent=`Average Glucose: ${avgSugar}`; ul.appendChild(li); box.appendChild(ul);
    }
